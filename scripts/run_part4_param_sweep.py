...
def run_sweep(run_root: Path, records: list[RunRecord], args: argparse.Namespace) -> None:
    ...
    while pending or running:
        while pending and len(running) < args.max_parallel:
            ...
            print(f"[start] {record.prediction_type}: {' '.join(record.command)}")
        ...
        for record, process, log_file in running:
            ...
            if exit_code == 0:
                ...
                print(f"[done] {record.prediction_type}")
            else:
                record.status = "failed"
                record.error = f"train.py exited with code {exit_code}"
                print(f"[failed] {record.prediction_type}: exit code {exit_code}")
            write_summary(run_root, records)
...
def main() -> None:
    ...
    print("=" * 72)
    print("Part IV parameterization sweep")
    print("=" * 72)
    print(f"Run root: {run_root}")
    print(f"Max parallel: {args.max_parallel}")
    print(f"Runs: {', '.join(record.prediction_type for record in records)}")
    print("=" * 72)
