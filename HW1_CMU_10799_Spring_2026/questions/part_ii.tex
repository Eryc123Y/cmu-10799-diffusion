...
\answerbox{}{
I evaluated the final EMA checkpoint using 1000 generated samples and 1000 real
images from the CelebA subset. The evaluation used \texttt{torch-fidelity} with
10 KID subsets of size 100. The generated and real image counts were both 1000.

\[
\mathrm{KID}_{\mathrm{mean}} = 0.006615829467773437,\qquad
\mathrm{KID}_{\mathrm{std}} = 0.001997329867895109.
\]

This is slightly above the suggested \(0.005\) target, but it is in the same
order of magnitude and the generated samples are visually face-like. The metric
was computed from the checkpoint \texttt{ddpm\_final.pt} with the command path
\texttt{scripts/evaluate\_kid.py}; the JSON output was written to
\texttt{/content/kid\_eval/kid\_metrics.json} in the Colab runtime.
}
\includeanswer{q4c}
